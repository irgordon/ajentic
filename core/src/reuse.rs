...
pub fn discover_reusable_candidates(
    ...
    if criteria.required_constraint_types.is_empty()
        || criteria.required_constraint_types.iter().any(|value| value.is_empty())
    {
    ...
    for (index, entry) in ledger.entries().iter().enumerate() {
        if let LedgerEntry::CandidateCreated(record) = entry {
            ...
            if !criteria.required_constraint_types.iter().any(|value| value == &record.constraints_id)
            {
                continue;
            }
...
pub fn build_reuse_applied_record(
    ...
    if reused_candidate_id.is_empty() || new_run_id.is_empty() {
        return Err(ReuseError::MissingDomainId);
    }
...
mod tests {
    ...
    fn invalid_criteria_rejected() {
        ...
        assert_eq!(discover_reusable_candidates(&criteria, &ledger), Err(ReuseError::MissingDomainId));
    }
    ...
    fn reuse_does_not_change_lifecycle_promotion_or_governance() {
        ...
        ledger.append(created("cand-1", "objective-a")).unwrap();
        ledger
            .append(LedgerEntry::EvaluationRecorded(EvaluationRecordedLedgerRecord {
            ...
            .unwrap();
        ledger
            .append(LedgerEntry::GovernanceReviewed(GovernanceReviewedLedgerRecord {
        ...
        let record = build_reuse_applied_record("reuse-1", "cand-1", "run-2", "manual", "operator", "ts-1").unwrap();
        ledger.append(LedgerEntry::ReuseApplied(record)).unwrap();

        let replay = crate::replay::replay_candidate("cand-1", &ledger).unwrap();
        assert_eq!(replay.final_status, crate::replay::ReplayFinalStatus::Unknown);
