...
mod tests {
    ...
    fn harness_request_fixture() -> LocalHarnessWorkflowRequest {
        LocalHarnessWorkflowRequest::new(
            "workflow-54",
            "run-54",
            "projection-54",
            "operator-54",
    ...
    #[test]
    fn local_harness_workflow_error_codes_are_stable() {
        assert_eq!(
            LocalHarnessWorkflowError::EmptyWorkflowId.code(),
            "empty_workflow_id"
        );
        assert_eq!(LocalHarnessWorkflowError::EmptyRunId.code(), "empty_run_id");
        assert_eq!(
            LocalHarnessWorkflowError::EmptyProjectionId.code(),
            "empty_projection_id"
        );
        assert_eq!(
            LocalHarnessWorkflowError::EmptyOperatorId.code(),
            "empty_operator_id"
        );
        assert_eq!(
            LocalHarnessWorkflowError::EmptyPromptSummary.code(),
            "empty_prompt_summary"
        );
        assert_eq!(
            LocalHarnessWorkflowError::EmptyContextPacketId.code(),
            "empty_context_packet_id"
        );
        assert_eq!(
            LocalHarnessWorkflowError::EmptyMemorySnapshotId.code(),
            "empty_memory_snapshot_id"
        );
    }

    #[test]
    fn local_harness_workflow_request_requires_workflow_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("", "r", "p", "o", "s", "c", "m"),
            Err(LocalHarnessWorkflowError::EmptyWorkflowId)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_run_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "", "p", "o", "s", "c", "m"),
            Err(LocalHarnessWorkflowError::EmptyRunId)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_projection_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "r", "", "o", "s", "c", "m"),
            Err(LocalHarnessWorkflowError::EmptyProjectionId)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_operator_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "r", "p", "", "s", "c", "m"),
            Err(LocalHarnessWorkflowError::EmptyOperatorId)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_prompt_summary() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "r", "p", "o", "", "c", "m"),
            Err(LocalHarnessWorkflowError::EmptyPromptSummary)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_context_packet_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "r", "p", "o", "s", "", "m"),
            Err(LocalHarnessWorkflowError::EmptyContextPacketId)
        );
    }
    #[test]
    fn local_harness_workflow_request_requires_memory_snapshot_id() {
        assert_eq!(
            LocalHarnessWorkflowRequest::new("w", "r", "p", "o", "s", "c", ""),
            Err(LocalHarnessWorkflowError::EmptyMemorySnapshotId)
        );
    }

    #[test]
    fn local_harness_workflow_completes_in_memory_when_gates_pass() {
        let result = run_local_harness_workflow(harness_request_fixture());
        assert_eq!(result.status, LocalHarnessWorkflowStatus::Completed);
        assert_eq!(result.reason, LocalHarnessWorkflowReason::CompletedInMemory);
    }

    #[test]
    fn local_harness_workflow_uses_stub_provider_only() {
        assert_eq!(
            run_local_harness_workflow(harness_request_fixture()).controlled_run_status,
            crate::execution::ControlledRunStatus::Accepted
        );
    }
    #[test]
    fn local_harness_workflow_provider_output_is_untrusted() {
        assert!(!run_local_harness_workflow(harness_request_fixture()).provider_output_trusted);
    }
    #[test]
    fn local_harness_workflow_provider_output_is_not_authoritative() {
        assert!(
            !run_local_harness_workflow(harness_request_fixture()).provider_output_authoritative
        );
    }
    #[test]
    fn local_harness_workflow_returns_read_projection_on_success() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .read_projection
            .is_some());
    }
    #[test]
    fn local_harness_workflow_summary_mentions_in_memory() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("completed in memory"));
    }
    #[test]
    fn local_harness_workflow_summary_mentions_no_real_provider_or_model_called() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("no real provider/model was called"));
    }
    #[test]
    fn local_harness_workflow_summary_mentions_no_files_read_or_written() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("no files were read or written"));
    }
    #[test]
    fn local_harness_workflow_summary_mentions_no_persistence() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("no persistence occurred"));
    }
    #[test]
    fn local_harness_workflow_summary_mentions_no_ui_or_api_transport() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("no UI or API transport was used"));
    }
    #[test]
    fn local_harness_workflow_does_not_claim_release_candidate_readiness() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("release-candidate readiness is not claimed"));
    }
    #[test]
    fn local_harness_workflow_does_not_claim_production_readiness() {
        assert!(run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("production readiness is not claimed"));
    }
    #[test]
    fn local_harness_workflow_does_not_call_persistence_plan() {
        assert!(!run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("physical_write_not_implemented"));
    }
    #[test]
    fn local_harness_workflow_does_not_execute_operator_intents() {
        assert!(!run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("operator intent"));
    }
    #[test]
    fn local_harness_workflow_does_not_mutate_ui_or_cli_dry_run() {
        assert!(!run_local_harness_workflow(harness_request_fixture())
            .summary
            .contains("dry-run command executed"));
    }

    #[test]
    fn local_harness_workflow_is_deterministic_for_same_request() {
        let request = harness_request_fixture();
        assert_eq!(
            run_local_harness_workflow(request.clone()),
